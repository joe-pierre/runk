...
## [CHOIX] Tâche 31 — Résolution de l'URL canonique TikTok (`canonicalUrl`), point d'entrée unique dans `TiktokProvider`

**Contexte :** un lien court TikTok (`vm.tiktok.com/XXXX`, fréquent depuis TikTok Lite, voir entrée « Tâche 16 » ci-dessus) ne contient pas l'identifiant vidéo — il n'apparaît que dans l'URL longue obtenue après résolution de la redirection HTTP. Sans cette URL longue, `DeepLinkService` ne peut pas construire de lien natif précis pointant vers la vidéo (préparation de la tâche suivante, deep link natif TikTok).
**Alternatives envisagées pour l'emplacement de la résolution :** (1) `ShareIntentService`/`ClipboardService`, au moment du partage/de la détection clipboard ; (2) `TiktokProvider` uniquement, au moment de la sauvegarde via `MetadataService`.
**Décision :** option 2, recommandée en amont et retenue ici — un seul point d'entrée réseau (les providers de `core/services/metadata/`), cohérent avec l'architecture en providers déjà actée (voir entrée « Architecture en providers... » ci-dessus) et avec SPEC.md section 5 (`metadata_service.dart` orchestre, ne contient aucune logique spécifique à une plateforme — aucune logique de résolution d'URL n'y est ajoutée, elle vit exclusivement dans `TiktokProvider`). Résoudre dès le partage aurait dupliqué un appel réseau (le lien est de toute façon re-visité à la sauvegarde) et étendu la responsabilité de `ShareIntentService`/`ClipboardService` au-delà de la détection/extraction d'URL qui leur est déjà propre.
**Détails d'implémentation :**
- La résolution suit manuellement les redirections HTTP (`Location` des réponses 3xx, `followRedirects: false` sur chaque `http.Request`) plutôt que de s'appuyer sur un éventuel suivi automatique d'un client HTTP réel — nécessaire pour rester déterministe et testable via `MockClient` (voir entrée « Tâche 4 — Mock HTTP des providers » ci-dessus, `http/testing.dart` ne simule jamais de suivi de redirection implicite, chaque `send()` déclenche une seule invocation du handler).
- Tourne en **parallèle** de l'appel oEmbed (les deux `Future` sont lancées avant le premier `await` dans `fetchMetadata`), avec son propre timeout interne (3s, volontairement plus court que le timeout global de 5s imposé par `MetadataService`, voir SPEC.md section 9) — évite qu'un lien court TikTok qui répond lentement à la résolution ne fasse échouer (ou ralentisse inutilement) toute la récupération de métadonnées, y compris quand l'oEmbed a réussi rapidement.
- Échec silencieux systématique (`try/catch` interne, jamais d'exception remontée) : timeout, erreur réseau, ou plus de 5 sauts de redirection (protection contre une boucle) laissent `canonicalUrl` à `null`, sans jamais affecter le résultat de l'appel oEmbed ni faire basculer `MetadataService` vers `GenericFallbackProvider`.
**Écart documenté par rapport au modèle figé dans SPEC.md section 3.1 :** `VideoMetadata.canonicalUrl` et `VideoBookmark.canonicalUrl` (nullable, `null` par défaut) ne sont pas reflétés dans SPEC.md — même traitement que `isHidden` (Tâche 22, non plus reflété dans SPEC.md 3.1) : le modèle de SPEC.md reste une référence figée au moment de sa rédaction, les écarts ultérieurs sont documentés ici plutôt que d'y être répercutés rétroactivement. Seul `TiktokProvider` renseigne ce champ pour l'instant ; tous les autres providers le laissent à `null` (contrainte explicite de la tâche, aucun autre provider modifié).
**Schéma Supabase :** `alter table bookmarks add column canonical_url text;` — à exécuter manuellement sur le projet Supabase (pas de dossier de migrations versionnées dans ce projet, voir TODO.md Phase 1 : le schéma initial a lui aussi été exécuté manuellement). `BookmarkRepository._toRemoteMap`/`_fromRemoteMap` mappent `canonicalUrl` ↔ `canonical_url`, avec valeur par défaut `null` côté lecture (`row['canonical_url'] as String?`) pour rester compatible avec les lignes déjà présentes avant l'ajout de la colonne.
**Isar :** `BookmarkEntity.canonicalUrl` (`String?`) ajouté à la collection existante — champ nullable, aucune migration Isar manuelle nécessaire ; `flutter pub run build_runner build --delete-conflicting-outputs` régénère le schéma sans erreur, les lignes déjà stockées se lisent avec `canonicalUrl == null` (vérifié par le test « pullRemoteChanges tolère l'absence de "canonical_url" sur une ligne distante existante avant cette tâche »).
**Leçon :** quand une tâche fournit déjà sa propre recommandation motivée pour une ambiguïté de conception (ici l'emplacement de la résolution), la valider explicitement dans DECISIONS.md plutôt que de re-décider silencieusement ou de la passer sous silence — cohérent avec la règle « toute ambiguïté est documentée, jamais tranchée silencieusement » même quand la réponse est déjà donnée.
**Statut :** ✅ Résolu — `lib/core/services/metadata/providers/tiktok_provider.dart`, `lib/core/services/metadata/video_metadata.dart`, `lib/features/bookmarks/domain/video_bookmark.dart`, `lib/features/bookmarks/data/bookmark_local_datasource.dart` (`BookmarkEntity`), `lib/features/bookmarks/data/bookmark_repository.dart`, `lib/features/bookmarks/presentation/add_bookmark_sheet.dart`. Colonne Supabase `bookmarks.canonical_url` restant à créer manuellement par l'utilisateur (SQL ci-dessus) avant que la synchronisation distante de ce champ ne soit effective — jusque-là, tout `insert`/`update` distant (utilisateur authentifié) échouera avec une erreur Postgres `column "canonical_url" does not exist` (42703), interceptée comme n'importe quel autre échec réseau par `_trySyncInsert`/`_trySyncUpdate` (`BookmarkRemoteSyncException`, voir entrée « Tâche 5 » ci-dessus) — le bookmark reste alors disponible localement avec `isSynced: false`, sans perte de données, mais ne remonte pas sur Supabase tant que la colonne n'existe pas.

---

## [CHOIX] Tâche 32 — Deep link natif TikTok/X-Twitter : format précis par id, plutôt que la reconstruction générique hôte/chemin héritée de la Tâche 8

**Contexte :** la reconstruction générique choisie en Tâche 8 (`scheme://host/path?query`) ouvre bien l'app cible pour TikTok et X/Twitter, mais sur son écran par défaut plutôt que sur le contenu partagé — ces deux plateformes attendent en réalité un format différent, avec un identifiant numérique précis extrait de l'URL, pas un simple recopiage hôte/chemin.
**Alternatives envisagées :** (1) conserver la reconstruction générique pour ces deux plateformes, en acceptant l'ouverture sur l'écran par défaut de l'app ; (2) formats spécifiques par id, vérifiés par la communauté (non documentés officiellement, mêmes réserves que le reste de la Tâche 8) : `twitter://status?id=<id>` (X/Twitter, id numérique après `/status/`) et `snssdk1233://aweme/detail/<id>?refer=web` (TikTok, id numérique après `/video/`).
**Décision :** option 2. `_nativeUriFor` extrait désormais l'id par regex (`/status/(\d+)` pour X/Twitter, `/video/(\d+)` pour TikTok) plutôt que de recopier hôte/chemin pour ces deux plateformes ; si l'id est introuvable (lien court TikTok non résolu, lien de profil sans statut, etc.), `_nativeUriFor` retourne `null` — même comportement que Threads/`unknown`, `openInSource` passe alors directement au repli navigateur existant, aucune nouvelle branche d'erreur nécessaire. Instagram/Facebook/YouTube inchangés (reconstruction générique déjà correcte pour ces plateformes, hors périmètre). Point d'appel `openBookmark` (`open_bookmark_action.dart`) modifié en conséquence : transmet désormais `bookmark.canonicalUrl ?? bookmark.url` plutôt que `bookmark.url` seul, pour bénéficier de l'URL longue déjà résolue par la Tâche 31 sur les liens TikTok courts déjà résolus au moment de la sauvegarde ; un lien court pas encore résolu (`canonicalUrl == null`) garde un comportement inchangé (id introuvable dans le lien court, repli navigateur sur ce même lien court).
**Leçon :** un format non documenté "observé fonctionner" (ouverture de l'app, Tâche 8) peut rester insuffisant si l'app cible interprète le chemin recopié comme une route inconnue plutôt que comme un identifiant de contenu précis — vérifier le format exact attendu par chaque app (id explicite vs simple chemin) plutôt que de généraliser un seul algorithme à toutes les plateformes dès qu'un doute existe sur le comportement observé en usage réel.
**Statut :** 🔵 Choix assumé — mêmes réserves de fragilité que la Tâche 8 (schémas non officiels, susceptibles de changer sans préavis, voir `BUGS_AND_ROADMAP.md`) — `lib/core/services/deep_link_service.dart`, `lib/features/bookmarks/presentation/open_bookmark_action.dart`.

---

## [RÉSOLU] Tâche 36 — Retour en arrière sur le schéma natif X/Twitter (`twitter://status?id=<id>`) : repli direct vers le lien https, comme Threads

**Contexte :** la Tâche 32 avait affiné le format natif X/Twitter (`twitter://status?id=<id>`, id extrait après `/status/`) pour corriger une ouverture sur l'écran par défaut de l'app plutôt que sur le post visé (reconstruction générique de la Tâche 8). Vérification manuelle sur appareil réel demandée pour cette tâche : l'app X s'ouvre bien, mais toujours sur l'accueil plutôt que sur le post visé — le format par id ne corrige donc pas le symptôme initialement visé par la Tâche 32 pour cette plateforme spécifiquement.
**Cause :** contrairement à l'hypothèse de la Tâche 32 (un format non documenté "observé fonctionner" suffirait s'il correspond à un identifiant de contenu plutôt qu'à un chemin), X déclare depuis 2016 que le schéma `twitter://` n'est ni documenté ni officiellement supporté, quel que soit le format de paramètres utilisé. Données externes convergentes citées par l'utilisateur (rapport Mobot, janvier 2025) : taux de succès très inégal et non fiable (~90 % Android, ~15 % iOS seulement) pour ce schéma, tous formats confondus.
**Alternatives envisagées :** (1) chercher un format de schéma `twitter://` alternatif encore plus précis — écarté, le problème n'est pas le format des paramètres mais l'absence de support officiel du schéma lui-même, quel que soit son format ; (2) abandonner toute tentative de schéma natif pour X/Twitter, repli direct et systématique vers le lien https d'origine (`bookmark.url`/`bookmark.canonicalUrl`), laissant l'OS gérer l'ouverture via Universal Links (iOS) / App Links (Android) — mécanisme officiellement associé au domaine `x.com`, donc structurellement plus fiable qu'un schéma personnalisé non documenté.
**Décision :** option 2. `_nativeUriFor` retourne désormais systématiquement `null` pour `VideoSource.twitter`, regroupé avec `VideoSource.threads`/`VideoSource.unknown` dans le switch de `deep_link_service.dart` — même traitement que Threads, qui n'a jamais eu de schéma natif connu. `_twitterStatusIdPattern` (devenu inutilisé, aucun autre appelant vérifié) est supprimé. `openInSource` n'est pas modifié : son repli navigateur existant (`launchUrl(Uri.parse(url), mode: LaunchMode.externalApplication)`) s'applique déjà immédiatement dès que `_nativeUriFor` retourne `null`, sans aucune tentative de schéma intermédiaire — comportement déjà correct pour Threads depuis la Tâche 8, maintenant partagé par X/Twitter. `<data android:scheme="twitter"/>` (AndroidManifest.xml) et l'entrée équivalente d'Info.plist restent en place (désormais inertes mais inoffensives) — nettoyage explicitement reporté à une passe de ménage séparée, hors périmètre de cette tâche.
**Leçon :** un schéma "observé fonctionner à l'ouverture de l'app" (Tâche 8) reste insuffisant à valider tant que le contenu précis visé n'est pas confirmé atteint sur device réel (Tâche 32 avait corrigé le format sans revalider ce point pour X/Twitter spécifiquement) — quand la plateforme cible déclare elle-même l'absence de support officiel d'un schéma (ici depuis 2016), aucun format de paramètres ne peut compenser cette absence de garantie ; le lien https natif (Universal/App Links) reste la seule voie officiellement fiable dans ce cas, quitte à renoncer entièrement à l'ouverture "directe dans l'app" via schéma personnalisé.
**Statut :** ✅ Résolu — `lib/core/services/deep_link_service.dart`, `test/unit/services/deep_link_service_test.dart`. Reste à revalider manuellement sur le même appareil qu'au signalement initial (ouverture directe sur le post visé, dépend de la configuration Universal Links/App Links côté OS et de l'app installée) — non réalisable dans cet environnement de développement, voir `BUGS_AND_ROADMAP.md`.

---

## [CHOIX] Révision Tâche 15 — `TagsScreen` : `FloatingActionButton` plutôt que bouton dans l'`AppBar`

**Contexte :** la Tâche 15 (voir entrée « Tâche 15 — Implémentation de la gestion indépendante des tags » ci-dessus) avait placé le bouton d'ajout de tag en `IconButton` dans les `actions` de l'`AppBar`. `HomeScreen` utilise de son côté un `FloatingActionButton` pour son action d'ajout équivalente (`ManualAddDialog.show`) — incohérence visuelle entre les deux écrans de premier niveau de l'application.

**Décision :** remplacement de l'`IconButton` de l'`AppBar` par un `FloatingActionButton` sur le `Scaffold` de `TagsScreen`, même icône (`Icons.add`) et même tooltip (`'Ajouter un tag'`, conservé pour ne pas casser `tags_screen_test.dart` qui cible le bouton via `find.byTooltip`). Aucun changement de comportement : `onPressed` continue d'appeler `_createTag(context, ref)` sans modification. Il ne s'agit pas d'un bug corrigé mais d'une révision assumée du choix visuel acté en Tâche 15, pour aligner `TagsScreen` sur le pattern déjà établi par `HomeScreen`.

**Vérification :** `flutter test test/widget/features/tags/presentation/tags_screen_test.dart` — 4 passed, 1 skipped (préexistant, sans rapport, voir entrée Tâche 10), tous verts sans aucune adaptation du fichier de test (aucun test ne ciblait `find.byType(IconButton)`).

**Statut :** 🔵 Choix assumé — `lib/features/tags/presentation/tags_screen.dart`.

---

## [CHOIX] Tâche 35 — Synchronisation distante des tags (`TagEntity`/`TagRepository`), extension de l'architecture de la Tâche 9

**Contexte :** `TagEntity` (Tâches 15/25) n'était synchronisé nulle part vers Supabase (voir l'ancienne doc de classe de `tag_repository.dart` : "Isar local uniquement") — un tag géré sans bookmark, ou un tag masqué, était perdu définitivement à la réinstallation. Branche `feat/tags-remote-sync`. Applique à la table `tags` la même architecture offline-first déjà validée pour `bookmarks` en Tâche 9 : push (`syncPendingChanges`) + pull (`pullRemoteChanges`) avec résolution **last-write-wins** sur `updated_at` (SPEC.md section 13), orchestrés par `SyncService`.

**Table Supabase :** déjà créée par l'utilisateur avant cette tâche, conforme au schéma demandé (`id`/`user_id`/`name`/`is_hidden`/`created_at`/`updated_at`, RLS activé, policy symétrique de `bookmarks`, `unique (user_id, name)`) — aucun écart constaté après vérification.

**`createdAt` ajouté à `TagEntity` :** oui, par cohérence avec `BookmarkEntity` et pour permettre un round-trip complet de la ligne distante (`tags.created_at`, qui a un `default now()` côté Postgres). N'est utilisé par aucun tri : `TagsScreen`/`distinctTagsProvider`/`hiddenTagsProvider` restent triés par ordre alphabétique (voir `distinct_tags_provider.dart`), inchangé.

**`TagEntity.remoteId` nullable, contrairement à `BookmarkEntity.remoteId` :** différence assumée et documentée dans la doc de classe de `TagEntity`. `BookmarkEntity.remoteId` a toujours existé depuis la Tâche 5 — chaque bookmark en a un dès sa création. `TagEntity`, elle, existe depuis la Tâche 15 *avant* cette extension : des tags gérés créés par des sessions précédentes n'ont jamais eu de `remoteId`. Ajouter le champ en non-nullable aurait nécessité une migration manuelle inexistante avec `isar_community` (voir DECISIONS.md, entrée "Tâche 31 — canonicalUrl" pour le précédent : un champ nullable ajouté à une collection existante ne nécessite aucune migration). `TagRepository` génère un `remoteId` à la volée au premier point d'écriture rencontré après cette tâche (`createTag`, `renameTag`, `hideTag`, `unhideTag` — backfill si `existingTag.remoteId == null`) ou, à défaut, au premier passage de `syncPendingChanges` (`entity.remoteId ??= _uuid.v4()`), avant tout `getAllPendingUpload`.

**`deleteTag` devient une suppression douce (`isDeletedLocally`), écart par rapport à la suppression Isar immédiate de la Tâche 15 :** l'ancienne implémentation appelait directement `_isar.tagEntitys.delete(...)` dans la transaction. Sans ce changement, un tag supprimé hors ligne aurait pu réapparaître localement après un `pullRemoteChanges` ultérieur (SPEC.md section 13, même risque déjà mitigé pour les bookmarks en Tâche 9) — puisqu'aucune trace locale de la suppression ne survivrait pour empêcher sa recréation. `deleteTag` marque désormais `isDeletedLocally: true`/`updatedAt`/`isSynced: false` dans la transaction (le retrait des bookmarks affectés reste inchangé), puis tente la suppression distante juste après ; en cas de succès (ou d'absence de session/`remoteId`), l'entité est retirée définitivement d'Isar. `TagLocalDatasource.findByName`/`getAll` excluent désormais systématiquement `isDeletedLocally: true` (même filtre que `BookmarkLocalDatasource.getAllActive`) — un tag en cours de suppression n'est donc jamais visible ni recréable par erreur entre-temps. **Limite mineure assumée, non résolue** : si un tag est soft-supprimé hors ligne puis qu'un nouveau tag de même nom est créé avant que la suppression distante ne soit confirmée, le conflit `unique (user_id, name)` côté Supabase fera échouer indéfiniment la synchronisation de l'un des deux tant que l'utilisateur ne revient pas en ligne pour purger la suppression en attente — scénario rare (nécessite d'être hors ligne au moment des deux actions), non traité différemment des autres échecs de sync (journalisé via `TagRemoteSyncException`, jamais silencieux).

**Simplification volontaire par rapport à `BookmarkRepository` — un seul `_trySyncUpsert`, pas de distinction insert/update :** `BookmarkRepository` distingue `_trySyncInsert`/`_trySyncUpdate` parce que `BookmarkRemoteDatasource.insert` est utilisé au moment de la création. Pour les tags, chaque point d'écriture (`createTag`, `renameTag`, `hideTag`, `unhideTag`, rattrapage de `syncPendingChanges`) peut indifféremment être une création ou une mise à jour distante (ex: un tag purement dérivé qui reçoit son premier `TagEntity` via `hideTag`) — un upsert idempotent (`TagRemoteDatasource.upsert`, déjà utilisé par `BookmarkRepository.syncPendingChanges` pour le même rattrapage générique) couvre tous les cas sans complexifier `TagRepository` d'une distinction qui n'apporte rien ici.

**Point d'attention explicite du prompt de tâche, vérifié plutôt que ré-implémenté :** la cascade `renameTag`/`deleteTag`/`hideTag`/`unhideTag` (Tâches 15/25) marquait déjà chaque `BookmarkEntity` affecté `isSynced: false` avant cette tâche — relecture du code confirmée avant toute modification. `SyncService`/`BookmarkRepository.syncPendingChanges` repoussent donc déjà normalement un bookmark dont les tags ont changé suite à un renommage/masquage de tag, sans logique supplémentaire nécessaire.

**Ordre dans `SyncService.syncNow()` — tags avant bookmarks :** choix documenté plutôt qu'imposé par une contrainte de correction stricte. Aucune intégrité référentielle entre les deux collections (`BookmarkEntity.tags` reste une simple liste de chaînes, jamais une clé étrangère vers `TagEntity`) : peu importe l'ordre, aucun état incohérent n'est possible. Les tags, jeu de données le plus léger, sont traités en premier pour une lecture du code plus intuitive.

**`getCurrentUserId`/`CurrentUserIdProvider` réutilisé tel quel depuis `bookmark_repository.dart` (`show CurrentUserIdProvider`), pas redéclaré :** même mécanisme que la Tâche 28, injecté par `tag_repository_provider.dart` exactement comme `bookmark_repository_provider.dart` (`() => SupabaseService.client.auth.currentUser?.id`) — contrainte explicite du prompt de tâche de ne dupliquer aucune logique déjà écrite pour `BookmarkRepository`.

**Rattachement rétroactif fusionné dans une seule confirmation, pas deux séquentielles :** `link_local_bookmarks_prompt.dart`/`promptToLinkLocalBookmarks` (Tâche 28) renommés en `link_local_data_prompt.dart`/`promptToLinkLocalData` — le nom d'origine serait devenu trompeur (CONVENTIONS.md, nommage explicite) une fois la fonction étendue aux tags. Alternative écartée : deux `AlertDialog` séquentiels (un pour les bookmarks, un pour les tags) à chaque connexion — rejetée comme régression d'UX pour un cas qui se produit toujours ensemble en pratique (un compte hors ligne accumule potentiellement les deux avant sa première connexion). La confirmation unique résume les deux comptes ("Associer N bookmark(s) et M tag(s) créés hors ligne à ce compte ?"), n'affiche la partie concernée que si son compte est non nul, et n'apparaît pas du tout si les deux sont à zéro (même garde-fou "vérifier avant d'afficher quoi que ce soit" que la Tâche 28).

**Vérification :** `dart run build_runner build --delete-conflicting-outputs` propre (nouveaux champs `TagEntity` : aucune migration manuelle requise, voir ci-dessus). `flutter analyze` propre. `flutter test -j 1` (suite complète) : 211 passed, 2 skipped (préexistants et sans rapport, voir entrée Tâche 10) — nouveaux tests : `tag_repository_test.dart` (nouveau groupe "synchronisation" : push authentifié/non authentifié, backfill de `remoteId` pour une entité historique, ordre suppression-avant-envoi, pull créé/last-write-wins/plus-ancien-ignoré/supprimé-ailleurs/suppression-locale-non-ressuscitée, rattachement rétroactif) ; `sync_service_test.dart` (nouveau cas : tags et bookmarks traités dans le même passage de `syncNow()`). `flutter test integration_test -d linux` toujours vert (flux Share Intent → Metadata → Save inchangé).

**Écart signalé :** aucun appareil physique Android/iOS ni émulateur disponible dans cette session (limitation déjà documentée pour la quasi-totalité des tâches précédentes) — le critère d'acceptation complet (créer/masquer un tag, se déconnecter, réinstaller, se reconnecter au même compte) n'a été vérifié qu'au niveau unitaire (Isar réel + fake du datasource distant, sans connexion Supabase réelle), pas de bout en bout sur device. À valider par l'utilisateur avant de considérer le critère d'acceptation pleinement confirmé.

**Leçon :** face à une extension d'une architecture déjà validée (Tâche 9, bookmarks) à une seconde collection existante et déjà peuplée par des sessions précédentes (`TagEntity` depuis la Tâche 15), le point de vigilance principal n'est pas de recopier le patron mais d'identifier ce qui diffère *parce que* la collection existait déjà avant l'extension — ici, la nullabilité de `remoteId` et le besoin d'un backfill explicite, contrairement à une collection introduite directement avec ses champs de synchronisation dès l'origine.

**Statut :** 🔵 Choix assumé — `lib/features/tags/data/tag_local_datasource.dart`, `tag_remote_datasource.dart` (nouveau), `tag_remote_sync_exception.dart` (nouveau), `tag_repository.dart`, `tag_repository_provider.dart`, `lib/features/bookmarks/data/sync_service.dart`, `sync_service_provider.dart`, `lib/features/auth/presentation/link_local_data_prompt.dart` (renommé depuis `link_local_bookmarks_prompt.dart`), `lib/app/app_drawer.dart`.

---

## [CHOIX] Tâche 34 — Style du bouton de sélection multiple : réutilisation des tokens de pastille de tag, pas de nouveau rôle `AppColorTokens`

**Contexte :** l'icône dédiée de l'`AppBar` de `HomeScreen` (`Icons.checklist`/`Icons.close`, Tâche 26) activant le mode sélection multiple était un `IconButton` par défaut, collé au bord droit de l'`AppBar` et peu visible. Branche `feat/selection-icon-style`.

**Décision :** ajout d'une marge droite de 12px (`Padding`, même valeur que les marges horizontales déjà utilisées ailleurs dans `HomeScreen`, ex. le champ de recherche) et d'un fond circulaire discret (`Container`/`BoxDecoration(shape: BoxShape.circle)`) autour de l'`IconButton` existant — aucun nouveau widget, aucune nouvelle logique. Le fond réutilise `AppColorTokens.tagBackground`/`.tagText` (rôles déjà définis pour les pastilles de tag, voir `app_color_tokens.dart`) plutôt que d'ajouter un nouveau rôle de couleur à `AppColorTokens` : la demande ("fond circulaire discret teinté selon le thème actif") correspond exactement au traitement visuel déjà conçu pour ces tokens, sans besoin d'un rôle supplémentaire. Nouvelle fonction privée `_colorTokens(context)` dans `home_screen.dart`, même pattern que `bookmark_card.dart` (fallback `AppColorTokens.dark` si aucun thème Runk enregistré, cas des tests de widget avec `MaterialApp` minimal). Traitement visuel strictement identique entre l'état actif et inactif — seule l'`Icon` (`Icons.checklist`/`Icons.close`) change, comme demandé. `tooltip` et `onPressed` (`BookmarkSelectionController.toggleSelectionMode`) inchangés.

**Alternative écartée :** ajouter un rôle dédié à `AppColorTokens` (ex. `iconButtonBackground`) pour ce seul usage — rejetée comme sur-ingénierie pour un unique bouton, alors qu'un rôle existant correspond déjà exactement au besoin visuel (fond pastel discret + texte/icône contrastés).

**Vérification :** `flutter analyze` propre. `flutter test` (suite complète, 211 passed, 2 skipped préexistants, voir entrée Tâche 10) — `bulk_selection_toolbar_test.dart` (qui cible cette icône via `find.byIcon(Icons.checklist)`/`find.byIcon(Icons.close)`) toujours vert sans adaptation, le `Container` englobant ne change ni le type d'`Icon` recherché par les tests ni la zone tapable de l'`IconButton`. Application lancée sur `linux` desktop (`flutter run -d linux`) : démarrage et rendu sans crash — aucun outil de capture d'écran disponible dans cet environnement pour une confirmation visuelle pixel par pixel, validation visuelle réelle laissée à l'utilisateur.

**Statut :** 🔵 Choix assumé — `lib/features/bookmarks/presentation/home_screen.dart`.

---

## [CHOIX] Tâche 35 — "Tout sélectionner" et checkboxes rondes

**Contexte :** Tâche 35, branche `feat/select-all-checkbox`. Deux ajouts au mode sélection multiple (Tâche 26) : une case "Tout sélectionner" au-dessus de la liste, et des checkboxes rondes plutôt que carrées dans toute l'app.

**Numérotation en collision, signalée plutôt que tranchée en silence :** `TODO.md` (Phase 6.7) contient déjà une entrée "Tâche 35 — Table Supabase `tags`..." (synchronisation distante des tags), cochée et non liée à celle-ci. Les deux tâches portent le même numéro dans l'historique du projet — écart non résolu ici (pas de renumérotation rétroactive tentée, risque de casser des références croisées existantes dans `DECISIONS.md`/`BUGS_AND_ROADMAP.md`). Cette entrée utilise "Tâche 35" tel que fourni par le prompt de tâche, en précisant systématiquement la branche (`feat/select-all-checkbox`) pour lever toute ambiguïté avec l'autre.

**`selectAll`/`deselectAll` dans `BookmarkSelectionController` :** `selectAll(List<String> ids)` remplace intégralement `selectedIds` (pas une union avec la sélection existante) — cohérent avec la sémantique "tout sélectionner" (jamais un ajout partiel). `deselectAll()` délègue à `clearSelection()` (déjà existante depuis la Tâche 26, identique) plutôt que de dupliquer sa logique — exposée sous un nom dédié pour que le code appelant (`home_screen.dart`) exprime explicitement l'intention symétrique de `selectAll` sans avoir à connaître `clearSelection`.

**Portée de "Tout sélectionner" limitée à la liste chargée, `null` pendant le chargement :** `HomeScreen` calcule désormais `currentBookmarks` une seule fois (`bookmarksAsync.maybeWhen(data: ..., orElse: () => null)`), réutilisé à la fois par la case "Tout sélectionner" et par le `data:` de `bookmarksAsync.when` (élimine la double évaluation du filtre qui existait avant cette tâche). Écart non explicite dans le prompt de tâche : la case "Tout sélectionner" reste masquée tant que `currentBookmarks` est `null` (chargement ou erreur), faute de liste concrète sur laquelle agir — alternative écartée (l'afficher désactivée) jugée inutile, ce cas n'est atteignable que par un rechargement pendant qu'une sélection est déjà active, fenêtre de temps très courte en pratique.

**Checkboxes rondes via `CheckboxThemeData` global (`AppTheme._build`), pas d'override local :** conforme à la demande explicite du prompt de tâche. Un seul point d'ajout (`shape: const CircleBorder()`) dans les deux thèmes (clair/sombre), sans toucher `bookmark_card.dart` ni `add_to_my_eyes_only_screen.dart` — les deux utilisaient déjà un `Checkbox`/`CheckboxListTile` standard, sans `shape` local à retirer.

**Test préexistant adapté (pas une régression) :** `bulk_selection_toolbar_test.dart` vérifiait `find.byType(Checkbox)` == exactement 2 (une par carte) en mode sélection avec 2 bookmarks — désormais 3, la case "Tout sélectionner" ajoutant une troisième instance de `Checkbox` (via `CheckboxListTile`). Assertion mise à jour (`findsNWidgets(3)` + vérification explicite du texte "Tout sélectionner"), les autres assertions `findsNothing` (hors mode sélection) restent valides telles quelles.

**Vérification :** `flutter analyze` propre. `flutter test -j 1` (suite complète) : vert, `bookmark_selection_controller_test.dart` (2 nouveaux cas : `selectAll`/`deselectAll`) et `home_screen_test.dart` (1 nouveau cas : case "Tout sélectionner" sélectionne/désélectionne la liste affichée, met à jour "Supprimer (N)"). Aucun appareil physique/émulateur disponible dans cette session (même limitation que la quasi-totalité des tâches précédentes) — rendu circulaire réel des checkboxes non confirmé visuellement, laissé à l'utilisateur.

**Statut :** 🔵 Choix assumé — `lib/features/bookmarks/presentation/bookmark_selection_controller.dart`, `lib/features/bookmarks/presentation/home_screen.dart`, `lib/core/theme/app_theme.dart`, `test/unit/features/bookmarks/presentation/bookmark_selection_controller_test.dart`, `test/widget/features/bookmarks/presentation/home_screen_test.dart`, `test/widget/features/bookmarks/presentation/bulk_selection_toolbar_test.dart`.

## [CHOIX] Tâche 36 — `ThemeModeSelector` : `Switch` Clair/Sombre + case "Suivre le système", en remplacement du `SegmentedButton`

**Contexte :** Tâche 36, branche `feat/theme-toggle-switch`. Réorganisation purement présentationnelle de `ThemeModeSelector` (Tâche 29) : un `Switch` à deux états (Clair/Sombre) associé à une `CheckboxListTile` séparée "Suivre le thème du système", sans toucher `theme_mode_provider.dart` (`ThemeModeController`, persistance `SharedPreferences`, défaut `ThemeMode.system`).

**Numérotation en collision, signalée plutôt que tranchée en silence :** `TODO.md` (Phase 5) contient déjà une entrée "Tâche 36 — Retour en arrière sur le schéma natif X/Twitter" (voir plus haut dans ce fichier), sans rapport avec celle-ci. Même situation que la collision "Tâche 35" documentée juste au-dessus — pas de renumérotation rétroactive tentée. Cette entrée utilise "Tâche 36" tel que fourni par le prompt de tâche, en précisant systématiquement la branche (`feat/theme-toggle-switch`) pour lever toute ambiguïté avec l'autre.

**Position du `Switch` quand `activeMode == ThemeMode.system` — ambiguïté non tranchée par le prompt :** `activeMode` vaut alors `ThemeMode.system`, ni `light` ni `dark` : aucune position "naturelle" pour le `Switch` grisé n'est déductible du seul état du controller. Choix (validé avec l'utilisateur) : dériver la position affichée de `MediaQuery.platformBrightnessOf(context)` (luminosité OS actuelle), plutôt que de mémoriser en état local le dernier `ThemeMode.light`/`.dark` explicite — évite de dupliquer un état déjà porté par `themeModeControllerProvider`, et respecte la contrainte du prompt ("montrer quel mode serait actif si on décochait"). Cette même position dérivée sert de base à `setThemeMode` quand on décoche "Suivre le système" (le `Switch` "redevient actif" exactement là où il était affiché, gris, l'instant d'avant).

**`Switch` désactivé via `onChanged: null`, pas de paramètre `enabled` :** `Switch` n'expose pas de propriété `enabled` dédiée en Flutter — passer `onChanged: null` le grise et bloque l'interaction, ce qui correspond exactement à la demande du prompt ("visible mais grisé, pas masqué").
...
## [CHOIX] Tâche 38 — `GenericWebsiteProvider` assigne `VideoSource.website` directement, sans passer par `SourceDetector`

**Contexte :** Tâche 38, branche `feat/generic-website-provider`. Jusqu'ici, `VideoSource` n'était jamais assignée que par `SourceDetector.detect` (voir DECISIONS.md, entrée "Tâche 4 — Emplacement de `VideoSource`") — y compris par `GenericFallbackProvider`, qui l'appelle explicitement pour retomber sur `unknown` si aucune des 6 plateformes n'est reconnue. Cette tâche introduit un niveau intermédiaire entre les 6 providers spécifiques et `GenericFallbackProvider` : `GenericWebsiteProvider`, qui tente un vrai scraping `og:` pour n'importe quel site web (blog, article, etc.), suivant le même pattern d'échec interne qu'Instagram/Facebook/Threads (voir entrée "Tâche 7 — Fallback interne aux providers de scraping").

**Problème :** `SourceDetector` ne reconnaît que les domaines des 6 plateformes vidéo et retombe sur `unknown` pour tout le reste (voir `source_detector.dart`) — il n'a par construction aucune notion de "site web générique" distincte d'"inconnu". Appeler `SourceDetector.detect(url)` depuis `GenericWebsiteProvider.fetchMetadata` en cas de succès du scraping aurait donc toujours renvoyé `VideoSource.unknown`, jamais `VideoSource.website` — ce nouveau cas ne peut pas être exprimé par `SourceDetector` sans lui apprendre une notion qui n'est pas la sienne (reconnaissance de domaine d'une plateforme vidéo précise).

**Alternatives envisagées :** (1) élargir `SourceDetector` pour qu'il retourne `website` dès qu'une URL est syntaxiquement valide et qu'aucune des 6 plateformes n'est reconnue — écarté, contredit la contrainte explicite de la tâche ("N'élargis pas `SourceDetector`") et briserait sa neutralité vis-à-vis du contenu réel de la page (une URL peut être valide et pourtant renvoyer une erreur HTTP ou n'avoir aucune balise `og:`, ce que `SourceDetector` ne peut pas savoir sans faire de requête réseau, hors de son rôle de fonction pure) ; (2) `GenericWebsiteProvider` assigne `VideoSource.website` lui-même, en dur, uniquement dans la branche de succès du scraping — cohérent avec le fait que c'est justement ce provider, et lui seul, qui sait qu'il vient de réussir à traiter un site générique.

**Décision :** option 2. `GenericWebsiteProvider.fetchMetadata` retourne `source: VideoSource.website` directement en cas de succès (titre `og:title` trouvé), et dans le résultat partiel de repli (échec de scraping) — dans les deux cas, c'est bien ce provider qui a traité l'URL, peu importe si le contenu a pu être extrait. Déroge donc légèrement à l'usage établi où `SourceDetector` est la seule source de vérité pour `VideoSource` — écart jugé nécessaire plutôt que silencieux, documenté ici comme demandé par le prompt de tâche.

**Conséquence :** `VideoSource.website` n'apparaît donc jamais en sortie de `SourceDetector.detect` (qui continue de retourner `unknown` pour toute URL hors des 6 plateformes, y compris les sites génériques) — seul `GenericWebsiteProvider` peut produire cette valeur. `GenericFallbackProvider` reste inchangé : il n'est atteint que si `GenericWebsiteProvider` lui-même échoue à un niveau que même son fallback interne ne couvre pas (uniquement le dépassement du timeout global de `MetadataService`, puisque `GenericWebsiteProvider.fetchMetadata` ne lève par construction aucune exception, seul cas restant après son propre repli interne), ou si son `canHandle` renvoie faux (URL invalide ou schéma non http/https) — dans ces cas restants, `GenericFallbackProvider` retombe sur `SourceDetector.detect`, donc sur `unknown`, jamais sur `website`.

**Registre des providers :** `GenericWebsiteProvider()` ajouté en dernière position de la liste par défaut de `MetadataService` (`metadata_service.dart`), après les 6 providers spécifiques — son `canHandle` accepte toute URL `http`/`https` valide, il ferait donc de l'ombre à n'importe quel provider placé après lui, et inversement serait lui-même ignoré s'il était placé avant un des 6 (leur ordre dans la liste prime, `_selectProvider` retourne le premier `canHandle` vrai).

**Effets de bord découverts en cours de tâche :** l'ajout de `VideoSource.website` à l'enum a rendu incomplets deux `switch` exhaustifs préexistants, tous deux mis à jour : `_svgAssetFor`/`_materialIconFor` dans `bookmark_card.dart` (nouveau cas → `Icons.public`, pas de SVG dédié, même traitement que YouTube/TikTok) et `DeepLinkService._nativeUriFor` (nouveau cas → `null`, regroupé avec Threads/X-Twitter/`unknown` : un site web quelconque n'a par nature aucune app associée, repli navigateur systématique).

**Limite préexistante, hors périmètre, révélée en écrivant le test de ce provider :** le regex partagé `OgTagScraper._contentAttributePattern` (`content=["']([^"']*)["']`) traite `'` comme un caractère de fin d'attribut valide même à l'intérieur d'une valeur délimitée par des guillemets doubles — un `og:title` contenant une apostrophe (français courant, ex. `l'article`) est donc tronqué à la première apostrophe rencontrée. Non corrigé ici : `OgTagScraper` est un utilitaire partagé (Instagram/Facebook/Threads/X déjà en Tâche 7/17), une correction devrait être vérifiée contre les quatre providers existants, hors périmètre de la Tâche 38. À traiter dans une tâche dédiée — voir `BUGS_AND_ROADMAP.md`.

**Vérification :** `flutter analyze` propre. `flutter test` (suite complète) : 219 passed, 2 skipped (préexistants, sans rapport, voir entrée Tâche 10). Aucun appareil physique/émulateur disponible dans cette session (même limitation que la quasi-totalité des tâches précédentes) — non vérifié manuellement sur device qu'un vrai lien d'article de blog produit bien la miniature/le titre attendus.

**Statut :** 🔵 Choix assumé — `lib/core/models/video_source.dart`, `lib/core/services/metadata/providers/generic_website_provider.dart` (nouveau), `lib/core/services/metadata/metadata_service.dart`, `lib/features/bookmarks/presentation/bookmark_card.dart`, `lib/core/services/deep_link_service.dart`.

---

## [CHOIX] Tâche 39 — Google Maps : aucune miniature récupérée, icône fixe assumée

**Contexte :** Tâche 39, branche `feat/maps-support`, suite directe de la Tâche 38 (`GenericWebsiteProvider`). Un lien Google Maps (lieu, itinéraire, lien court `maps.app.goo.gl`) partagé vers Runk n'est pas une vidéo — c'est un cas à part parmi les URLs non reconnues par les 6 plateformes vidéo, jusqu'ici absorbé silencieusement par `GenericWebsiteProvider` (Tâche 38) comme n'importe quel site web générique.

**Problème :** les pages Google Maps sont fortement rendues côté client en JavaScript. Un scraping `og:image` classique (la technique déjà utilisée par `OgTagScraper`, partagée avec Instagram/Facebook/Threads/`GenericWebsiteProvider`) n'y récupère pas de façon fiable une image représentative du lieu — contrairement à `og:title`, généralement présent et fiable (nom du lieu). Laisser `GenericWebsiteProvider` traiter ces liens produirait donc un flot de bookmarks avec titre correct mais `isPartial: true`/placeholder d'échec systématique pour la miniature, alors que ce n'est pas un échec ponctuel de scraping mais une caractéristique structurelle de ce type de page.

**Alternatives envisagées (discutées en amont avec l'utilisateur, avant l'écriture de ce prompt) :**
1. Scraping `og:image`/autre balise sur la page Maps malgré la limitation connue — écarté, taux d'échec attendu proche de 100 %, ne ferait que déguiser un `isPartial` quasi systématique.
2. Appel à l'API Google Maps Static Maps pour générer une vraie image de carte à la volée — écarté : nécessite une clé API payante (quota/facturation à gérer, contredit SPEC.md section 9 "aucune clé secrète en dur"/gestion de coûts non justifiée pour ce périmètre) et introduit une dépendance externe supplémentaire pour un cas d'usage secondaire (Runk reste une app de sauvegarde de vidéos, Maps est un à-côté).
3. Aucune récupération d'image pour cette source : une icône Material fixe (`Icons.location_on`) suffit à signaler visuellement "ceci est un lieu", sans scraping ni API tierce.

**Décision :** option 3, validée explicitement par l'utilisateur. `VideoSource.maps` (nouvelle valeur d'enum) ne récupère **jamais** de miniature — `MapsProvider.fetchMetadata` n'extrait ni n'assigne `VideoMetadata.thumbnailUrl`, même si `og:image` est présent dans la réponse de `OgTagScraper` (contrairement à `GenericWebsiteProvider`, qui l'utilise quand disponible). Seul `og:title` est récupéré. `BookmarkCard._BookmarkThumbnail` affiche pour cette source une icône fixe (`Icons.location_on`, fond `colorScheme.primary` non cyclique) au lieu de toute tentative de miniature réseau.

**Dérogation documentée au sens habituel d'`isPartial` (SPEC.md section 3.1) :** `isPartial` signifie normalement "les métadonnées n'ont pas pu être récupérées entièrement" — un signal de dégradation/échec. Pour `VideoSource.maps`, l'absence de miniature est **structurelle et voulue**, pas un échec : `MapsProvider` retourne donc `isPartial: false` dès que le titre est récupéré avec succès, malgré `thumbnailUrl` toujours `null`. `isPartial: true` n'est réservé qu'au cas où même le titre échoue (repli sur `MapsProvider.defaultTitle`, `'Lieu sans titre'`) — c'est le seul signal de dégradation encore pertinent pour cette source. Écart assumé et documenté ici comme demandé par le prompt de tâche, plutôt que de forcer `isPartial: true` systématiquement (ce qui aurait fait apparaître le placeholder gris d'échec habituel sur `BookmarkCard`, contredisant justement l'objectif visuel de l'icône fixe).

**Détection de domaine (`SourceDetector`, seule classe modifiée pour la reconnaissance de domaine, conformément à sa doc de classe) :** formats vérifiés avant écriture du code (recherche web, aucun appareil physique disponible pour un test de partage réel depuis l'app mobile Google Maps dans cet environnement — limitation habituelle de cette session) :
- `maps.app.goo.gl/<id>` — lien court actuel généré par le bouton "Partager" de l'app mobile (remplace progressivement l'ancien format court) ;
- `goo.gl/maps/<id>` — ancien format court, toujours en circulation sur des liens plus anciens ;
- `google.com/maps/place/...`, `google.com/maps?cid=...` — formats longs (partage depuis le web ou "Copier les coordonnées"/lien direct).

`google.com`/`www.google.com` n'est reconnu comme Maps **que** si `Uri.path` commence par `/maps` — le domaine seul sert aussi à la recherche web classique (`google.com/search?...`), qui doit impérativement rester `unknown` et continuer d'être pris en charge par `GenericWebsiteProvider` (Tâche 38), pas par `MapsProvider`. Testé explicitement (`source_detector_test.dart`, cas "google.com sans chemin /maps reste unknown").

**Registre des providers :** `MapsProvider()` inséré dans `MetadataService` **avant** `GenericWebsiteProvider`, après les 6 providers vidéo spécifiques — même raison que pour `GenericWebsiteProvider` lui-même vis-à-vis des 6 providers (Tâche 38) : le `canHandle` de `GenericWebsiteProvider` accepte toute URL `http`/`https` valide, il intercepterait donc un lien Maps avant que celui-ci n'atteigne `MapsProvider` s'il était placé avant.

**Hors périmètre, explicitement écarté par le prompt de tâche :** aucun deep link natif `comgooglemaps://` — `DeepLinkService._nativeUriFor` retourne `null` pour `VideoSource.maps`, regroupé avec Threads/X-Twitter/`website`/`unknown` (repli navigateur systématique, comportement par défaut déjà en place, aucune régression sur les autres sources).

**Effet de bord :** l'ajout de `VideoSource.maps` à l'enum a rendu incomplets les deux mêmes `switch` exhaustifs déjà mis à jour en Tâche 38 : `_svgAssetFor`/`_materialIconFor` dans `bookmark_card.dart` (nouveau cas → pas de SVG dédié, `Icons.location_on`) et `DeepLinkService._nativeUriFor` (nouveau cas → `null`).

**Vérification :** `flutter analyze` propre. Tests unitaires ajoutés : `source_detector_test.dart` (détection des 4 formats Maps + non-régression `google.com/search`), `maps_provider_test.dart` (nouveau, 4 cas : `canHandle` positif/négatif, `thumbnailUrl` toujours `null` même si `og:image` présent, repli `isPartial: true` si titre absent/scraping en échec). `flutter test -j 1` (suite complète) : 226 passed, 2 skipped (préexistants, sans rapport, voir entrée Tâche 10). Aucun appareil physique/émulateur disponible dans cette session (même limitation que la quasi-totalité des tâches précédentes) : le critère d'acceptation exact (partage réel depuis le bouton "Partager" de l'app mobile Google Maps) n'a été vérifié qu'au niveau unitaire (mock HTTP) et par recherche des formats d'URL documentés/observés publiquement — à valider par l'utilisateur avant de considérer la tâche pleinement confirmée.

**Statut :** 🔵 Choix assumé — `lib/core/models/video_source.dart`, `lib/core/utils/source_detector.dart`, `lib/core/services/metadata/providers/maps_provider.dart` (nouveau), `lib/core/services/metadata/metadata_service.dart`, `lib/features/bookmarks/presentation/bookmark_card.dart`, `lib/core/services/deep_link_service.dart`.
